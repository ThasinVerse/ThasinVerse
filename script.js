...
/* =========================================================
   INITIALIZE (Updated - Loading Screen added)
========================================================= */
    ...
    container.innerHTML = `
        <div class="learning-path reveal">
            <div class="path-header">
                <div>
                    <span class="path-label">SELECT YOUR LEVEL</span>
                    <h3>তোমার Academic Level নির্বাচন করো</h3>
                </div>
                <div class="path-icon">
            ...
            <div class="class-grid">
                ${createClassCard("class6", "Class 6", "ষষ্ঠ শ্রেণি", "school", "fa-atom", "Subject Select")}
                ${createClassCard("class7", "Class 7", "সপ্তম শ্রেণি", "school", "fa-flask", "Subject Select")}
                ${createClassCard("class8", "Class 8", "অষ্টম শ্রেণি", "school", "fa-brain", "Subject Select")}
                ${createClassCard("ssc", "SSC", "Secondary School Certificate", "exam", "fa-graduation-cap", "Subject Select")}
                ${createClassCard("hsc", "HSC", "Higher Secondary Certificate", "exam", "fa-user-graduate", "Subject Select")}
            </div>
        </div>
    `;
...
function createClassCard(key, title, subtitle, type, icon, action) {
    return `
        <button class="class-card sound-btn" data-sound="open" onclick="selectClass('${key}')">
            <div class="class-card-glow"></div>
            <div class="class-card-top">
                <div class="class-icon">
                    <i class="fa-solid ${icon}"></i>
                </div>
                <span class="class-type">${type === "exam" ? "EXAM LEVEL" : "SCHOOL LEVEL"}</span>
            </div>
            ...
            <div class="card-number">${key.replace("class", "").toUpperCase()}</div>
        </button>
    `;
}
...
function showSubjectSelection(classKey) {
    ...
    container.innerHTML = `
        <div class="inner-navigation">
            <button onclick="showClasses()" class="back-button sound-btn">
        ...
        <div class="subject-grid">
            ${Object.entries(data.subjects).map(([subject, info]) =>
                createSubjectCard(classKey, subject, info)
            ).join("")}
        </div>
    `;
    ...
    return `
        <button class="subject-card sound-btn" data-sound="open" onclick="openSubject('${classKey}', '${escapeQuotes(subject)}')">
            ...
            <div class="subject-details">
                <span class="subject-small">SUBJECT</span>
                <h3>${subject}</h3>
                <p>${count > 0 ? `${count}টি ভিডিও available` : "ভিডিও শিগগিরই আসছে"}</p>
            </div>
...
function createVideoCard(video) {
    ...
    return `
        <div onclick="openVideoModal('${video.url}')" class="video-card sound-btn" data-sound="success" role="button" tabindex="0" aria-label="Play video: ${escapeHTML(video.title)}">
...
function setupSoundSystem() {
    ...
    document.addEventListener("click", event => {
        const element = event.target.closest(".sound-btn");
        if (!element || element.id === "soundToggle") return;

        const soundType = element.dataset.sound || "click";
...
/* =========================================================
   REVEAL ANIMATION (Enhanced - Multiple Types)
========================================================= */
...
/* =========================================================
   THASINVERSE PREMIUM LEARNING ENGINE
   Version: 5.1 (Fixed Loading Screen)
========================================================= */
            ...
            "Physics": {
                ...
                chapters: {
                    ...
                    "অধ্যায় ৪: কাজ, ক্ষমতা ও শক্তি": {
                        ...
                        basic: [],
                        oneshot: [
                            { title: "কাজ, ক্ষমতা ও শক্তি Final shot! 😳 | কাজ, ক্ষমতা ও শক্তি Oneshot | SSC 2026", url: "https://youtu.be/XrkGCqvZhcc" }
                        ]
                    },
                    "অধ্যায় ৫: পদার্থের অবস্থা ও চাপ": {
                        academic: [],
                        basic: [
                            { title: "🔹 ৭৬ cm পারদ = ১ atm কেন? | পারদের চাপ | Atmospheric Pressure", url: "https://youtu.be/KtaCP4xhFrA" }
                        ],
                        oneshot: []
                    },
                    ...
                    "অধ্যায় ৭: তরঙ্গ ও শব্দ": {
                        ...
                        basic: [],
                        oneshot: [
                            { title: "ফিজিক্স ৭ অধ্যায়ের Final shot! 😳 | তরঙ্গ ও শব্দ Oneshot | SSC 2026", url: "https://youtu.be/GbkZK74Hq1M" }
                        ]
                    },
                    ...
                    "অধ্যায় ১০: স্থির তড়িৎ": {
                        ...
                        oneshot: [
                            { title: "স্থির বিদ্যুৎ Final shot! 😳 | স্থির বিদ্যুৎ Oneshot | SSC Physics chapter 10", url: "https://youtu.be/ba2QQRdjl4s" }
                        ]
                    },
                    "অধ্যায় ১১: চল তড়িৎ": {
                        academic: [],
            ...
            "Chemistry": {
                ...
                chapters: {
                    ...
                    "অধ্যায় ৬: মোলের ধারণা ও রাসায়নিক গণনা": {
                        academic: [],
                        basic: [
                            { title: "১ মোল আসলে কত? | অ্যাভোগাড্রোর সংখ্যা 6.022×10²³ সহজ ব্যাখ্যা", url: "https://youtu.be/Q8nRvIInk-4" }
                        ],
                        oneshot: []
                    },
                ...
                chapters: {
                    ...
                    "অধ্যায় ৭: অসীম ধারা": {
                        ...
                        basic: [
                            ...
                            { title: "📘 SSC Math: অসীম সংখ্যা শেষবারের মতো ক্লিয়ার করো!", url: "https://youtu.be/bgMRUmhwRyI" }
                        ],
                        oneshot: []
                    },
            ...
            "General Math": {
                ...
                chapters: {
                    ...
                    "অধ্যায় ৪: সূচক ও লগারিদম": {
                        academic: [],
                        basic: [
                            { title: "বৈজ্ঞানিক পদ্ধতিতে সংখ্যার প্রকাশ | সহজভাবে ব্যাখ্যা", url: "https://youtu.be/aafAMZiQGkQ" }
                        ],
                        oneshot: []
                    },
    ...
    "hsc": {
        ...
        subjects: {
            ...
            "Biology": {
                icon: "fa-dna",
                videos: [
                    { title: "HSC 2027 Translation | mRNA থেকে Protein কীভাবে তৈরি হয়?", url: "https://youtu.be/wX-LK8ou0W8" }
                ]
            },
            "Chemistry": {
                icon: "fa-flask",
...
function showClasses() {
    ...
    container.innerHTML = '<div class="learning-path reveal"><div class="path-header"><div><span class="path-label">SELECT YOUR LEVEL</span><h3>তোমার Academic Level নির্বাচন করো</h3></div><div class="path-icon"><i class="fa-solid fa-layer-group"></i></div></div><div class="class-grid">' +
        createClassCard("class6", "Class 6", "ষষ্ঠ শ্রেণি", "school", "fa-atom", "Subject Select") +
        createClassCard("class7", "Class 7", "সপ্তম শ্রেণি", "school", "fa-flask", "Subject Select") +
        createClassCard("class8", "Class 8", "অষ্টম শ্রেণি", "school", "fa-brain", "Subject Select") +
        createClassCard("ssc", "SSC", "Secondary School Certificate", "exam", "fa-graduation-cap", "Subject Select") +
...
function createClassCard(key, title, subtitle, type, icon, action) {
    return '<button class="class-card sound-btn" data-sound="open" onclick="selectClass(\'' + key + '\')">' +
        ...
        '<span class="class-type">' + (type === "exam" ? "EXAM LEVEL" : "SCHOOL LEVEL") + '</span></div>' +
...
function showSubjectSelection(classKey) {
    ...
    var subjectsHTML = "";
    var subjects = Object.entries(data.subjects);
    for (var i = 0; i < subjects.length; i++) {
        subjectsHTML += createSubjectCard(classKey, subjects[i][0], subjects[i][1]);
    }

    container.innerHTML = '<div class="inner-navigation"><button onclick="showClasses()" class="back-button sound-btn"><i class="fa-solid fa-arrow-left"></i> All Classes</button></div>' +
        '<div class="selection-header reveal"><div class="selection-icon' + iconExtraClass + '"><i class="fa-solid ' + iconClass + '"></i></div>' +
        '<div><span>' + data.title + '</span><h2>' + headingText + '</h2><p>' + descriptionText + '</p></div></div>' +
...
function createSubjectCard(classKey, subject, info) {
    ...
    if (info.chapters) {
        var chapters = Object.values(info.chapters);
        for (var i = 0; i < chapters.length; i++) {
            count += chapters[i].academic.length + chapters[i].basic.length + chapters[i].oneshot.length;
        }
    } else if (info.videos) {
        count = info.videos.length;
    }
    ...
    return '<button class="subject-card sound-btn" data-sound="open" onclick="openSubject(\'' + classKey + '\', \'' + escapeQuotes(subject) + '\')">' +
        ...
        '<p>' + (count > 0 ? count + 'টি ভিডিও available' : 'ভিডিও শিগগিরই আসছে') + '</p></div>' +
...
function showChapterList(classKey, subject) {
    ...
    var chapters = Object.entries(data.chapters);
    var chapterCount = chapters.length;

    var chaptersHTML = "";
    for (var i = 0; i < chapters.length; i++) {
        chaptersHTML += createChapterCard(classKey, subject, chapters[i][0], chapters[i][1]);
    }

    container.innerHTML = '<div class="inner-navigation"><button onclick="showSubjectSelection(\'' + classKey + '\')" class="back-button sound-btn"><i class="fa-solid fa-arrow-left"></i> Subjects</button></div>' +
        '<div class="selection-header reveal"><div class="selection-icon"><i class="fa-solid ' + data.icon + '"></i></div>' +
        '<div><span>' + subject + '</span><h2>Chapter List</h2><p>' + educationData[classKey].title + ' - ' + subject + ' এর ' + chapterCount + 'টি অধ্যায়</p></div></div>' +
...
function createChapterCard(classKey, subject, chapter, info) {
    ...
    return '<button class="chapter-card sound-btn" data-sound="open" onclick="showClassTypeOptions(\'' + classKey + '\', \'' + escapeQuotes(subject) + '\', \'' + escapeQuotes(chapter) + '\')">' +
        ...
        '<div class="chapter-card-content"><h3>' + chapter + '</h3><p>' + (totalVideos > 0 ? totalVideos + 'টি ভিডিও available' : 'ভিডিও শিগগিরই আসছে') + '</p></div>' +
        '<div class="chapter-card-bottom"><span>View Classes</span><i class="fa-solid fa-arrow-right"></i></div>' +
        '</button>';
}
...
function showClassTypeOptions(classKey, subject, chapter) {
    ...
    container.innerHTML = '<div class="inner-navigation"><button onclick="showChapterList(\'' + classKey + '\', \'' + escapeQuotes(subject) + '\')" class="back-button sound-btn"><i class="fa-solid fa-arrow-left"></i> Chapters</button></div>' +
        '<div class="selection-header reveal"><div class="selection-icon"><i class="fa-solid fa-list-check"></i></div>' +
        '<div><span>' + subject + ' — ' + chapter + '</span><h2>Class Type নির্বাচন করো</h2><p>তোমার পছন্দমতো class type বেছে নাও</p></div></div>' +
        '<div class="class-type-grid">' +
        createClassTypeCard("academic", "Academic Class", "fa-graduation-cap", "বিস্তারিত ও গভীর পাঠ", data.academic.length) +
        createClassTypeCard("basic", "Basic Class", "fa-lightbulb", "সহজ ও মৌলিক পাঠ", data.basic.length) +
        createClassTypeCard("oneshot", "Oneshot Class", "fa-bolt", "দ্রুত সম্পূর্ণ রিভিশন", data.oneshot.length) +
...
function createClassTypeCard(type, title, icon, description, videoCount) {
    return '<button class="class-type-card sound-btn" data-sound="open" onclick="openClassType(\'' + type + '\')">' +
        ...
        '<span class="video-badge">' + (videoCount > 0 ? videoCount + 'টি ভিডিও' : 'শিগগিরই আসছে') + '</span></div>' +
        '<div class="class-type-arrow"><i class="fa-solid fa-chevron-right"></i></div>' +
        '</button>';
}
...
function showVideoList(title, subtitle, videos, backFunction) {
    ...
    container.innerHTML = '<div class="inner-navigation"><button onclick="' + backAction + '" class="back-button sound-btn"><i class="fa-solid fa-arrow-left"></i> Back</button></div>' +
        '<div class="selection-header reveal"><div class="selection-icon"><i class="fa-solid fa-play"></i></div>' +
        '<div><span>' + title + '</span><h2>Video Classes</h2><p>' + subtitle + '</p></div></div>' +
        videosHTML;
